...
OFSPathDiscover::OFSPathDiscover() {
    ...
    for ( auto &p : paths ) {
        ...
        if ( fs::exists(p) ) {
            ...
            if ( fs::exists( p / "steamapps/common/Source SDK Base 2013 Multiplayer" ) ) {
                std::cout << "Path also contains Source SDK dir" << std::endl;
                sourcePath = p / "steamapps/common/Source SDK Base 2013 Multiplayer";
            }
        }
    }
}
