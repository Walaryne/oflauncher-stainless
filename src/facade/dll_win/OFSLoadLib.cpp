...
void ShowError(const std::string &msg) {
	...
	MessageBox(nullptr, err.c_str(), msg.c_str(), MB_OK);
}

void *realClientInterface(char const *name, int *err) {
	typedef void *(*cInt)(char const *name, int *err);
	// HINSTANCE handle =
	// LoadLibraryEx("C:\\Users\\Fenteale\\Documents\\Projects\\ofd\\Open-Fortress-Source\\game\\open_fortress\\bin\\client_real.dll",
	// NULL, LOAD_WITH_ALTERED_SEARCH_PATH);  //this works
	HINSTANCE handle =
